...
class LinearSolver:
    ...
    def __init__(self, assembler: StokesAssembler):
        """Initialize."""
    ...
    def direct_lu_solve(
        ...
        """Direct solve using direct sparse LU (splu)."""
        if (key := "direct_lu") not in self._ab_cache:
            logger.debug("Assembling system matrix and RHS for direct LU")
            A, b = self.assembler.assemble()
        ...
        if key not in self._lu_cache:
            logger.debug("Factoring system matrix.")
